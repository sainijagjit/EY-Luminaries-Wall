...
import { getVideoSrc } from '../constants/assets';
import {
  ANIMATION_INTERVAL,
  GROUP_ANIMATION_DELAYS,
  generateRandomIndices,
  buildCharacterGroupsFromConfig,
  getCharacterZIndex,
} from '../utils/dashboardUtils';

export default function Dashboard({ logoAnimationComplete, config }) {
  const dashboardScreen = useMemo(
    () =>
      config?.screens?.find((screen) => screen.screen_name === 'dashboard') ||
      null,
    [config],
  );

  const characterGroups = useMemo(
    () => buildCharacterGroupsFromConfig(dashboardScreen?.visualizations),
    [dashboardScreen],
  );
  ...
  const renderCharacterImage = (character, index, groupIndex) => {
    ...
    const videoSrc = getVideoSrc(character.video);
